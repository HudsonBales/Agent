...
import { Insight, MetricSeries, Anomaly } from "../types";
import { LLMService } from "../core/llm-service";

export class OpsIntelligenceAgent {
  private llm?: LLMService;

  constructor(private signals: SignalsService, apiKey = process.env.OPENAI_API_KEY) {
    if (!apiKey) {
  ...
  async generateInsights(workspaceId: string): Promise<Insight[]> {
    ...
    try {
      const result = await this.llm.generateStructuredOutput<{ insights: { text: string; category: string }[] }>(
        prompt,
        systemPrompt
      );
      ...
      return result.insights.map((insight, index) => ({
        ...
        workspaceId,
        text: insight.text,
        category: insight.category as "metric" | "incident" | "suggestion",
  ...
  private buildFallbackInsights(workspaceId: string, metrics: MetricSeries[], anomalies: Anomaly[]) {
