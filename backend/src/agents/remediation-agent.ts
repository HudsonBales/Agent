...
export class RemediationAgent {
  ...
  constructor(private workflows: WorkflowEngine, apiKey = process.env.OPENAI_API_KEY) {
    if (!apiKey) {
  ...
  async suggest(workspaceId: string, anomalies: Anomaly[]): Promise<RemediationSuggestion[]> {
    ...
    try {
      const result = await this.llm.generateStructuredOutput<{ suggestions: RemediationSuggestion[] }>(
        prompt,
        systemPrompt
      );
      ...
      return result.suggestions.filter((suggestion) =>
        definitions.some((w) => w.id === suggestion.workflowId)
      );
    } catch (error) {
  ...
  private buildFallback(definitions: ReturnType<WorkflowEngine["listDefinitions"]>, anomalies: Anomaly[]) {
